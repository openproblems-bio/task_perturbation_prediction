...
def split_over_all_test(df):
    valid_folds = []
    sm_names_B_M = pd.unique(df[df['cell_type'].isin(['B cells', 'Myeloid cells'])]['sm_name'])
    for cell_type in ['NK cells', 'T cells CD4+', 'T cells CD8+', 'T regulatory cells']:
        sub_df = df[(df['cell_type'] == cell_type) & (~df['sm_name'].isin(sm_names_B_M))]
...
def split_over_all_test(df, df_orig):
    valid_folds = []
    sm_names_B_M = pd.unique(df_orig[df_orig['cell_type'].isin(['B cells', 'Myeloid cells'])]['sm_name'])
    for cell_type in ['NK cells', 'T cells CD4+', 'T cells CD8+', 'T regulatory cells']:
        sub_df = df[(df['cell_type'] == cell_type) & (~df['sm_name'].isin(sm_names_B_M))]
        valid_folds.append(sub_df.index.tolist())
    folds = []
...
models = [model_1] + [model_2] * 3 + [model_3] * 2 + [model_4] * 2 + [model_5] * 3 + [model_6] * 3 + [model_7] * 2 + [model_8] * 4
params = [params_model_1a,
...
weights = {'w_0': 0.039503611057797205, 'w_1': 0.22778329445024798, 'w_2': 0.6470451053292054, 'w_3': 0.14998376919294348,
...
def predict(test_df, models, params, weights):
    ...
    for modeli in range(len(models)):
        ...
        for i in range(par["reps"]):
            print(f"Training model {modeli}/{len(models)}, repeat {i}/{par['reps']}", flush = True)
            temp_pred.append(fit_and_predict_embedding_nn(new_names, original_y, x_test, model, param))
