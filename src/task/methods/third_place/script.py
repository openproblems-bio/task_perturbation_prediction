...
for model, param in zip(models, params):
    temp_pred = [
        fit_and_predict_embedding_nn(new_names, original_y, x_test, model, param)
        for i in range(reps)
    ]
...
output = pd.DataFrame(
    clipped_pred, index=test_df["id"], columns=gene_names
).reset_index()

print("Write output to file", flush=True)
output.to_parquet(par["output"])
output.to_csv(par["output"].replace("parquet", "csv"), index=False)
