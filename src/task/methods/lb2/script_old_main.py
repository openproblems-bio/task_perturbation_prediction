...
class CustomTransformer(nn.Module):
    def __init__(self, num_features, num_labels, d_model=128, num_heads=8, num_layers=6):  # num_heads=8
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=d_model, nhead=num_heads, device='cpu', dropout=0.3,
                                       activation=nn.GELU(),
...
class CustomTransformer_v3(nn.Module):  # mean + std
    def __init__(self, num_features, num_labels, d_model=128, num_heads=8, num_layers=6, dropout=0.3):
        ...
        print("old sparse feature")
        print(self.num_sparse_features)
...
def train_func(X_train, Y_reduced, X_val, Y_val, n_components, num_epochs, batch_size, label_reducer, scaler,
               d_model=128, early_stopping=5000, device='cpu', ):
...
def train_transformer_k_means_learning(X, Y, n_components, num_epochs, batch_size,
                                       d_model=128, early_stopping=5000, device='cpu', seed=18):
...
def train_k_means_strategy(n_components_list, d_models_list, one_hot_encode_features, targets, num_epochs,
    ...
    for n_components in n_components_list:
        for d_model in d_models_list:
            ...
            os.makedirs('trained_models_kmeans', exist_ok=True)
            # Save the trained models
            with open(f'trained_models_kmeans/label_reducer_{n_components}_{d_model}.pkl', 'wb') as file:
                pickle.dump(label_reducer, file)

            with open(f'trained_models_kmeans/scaler_{n_components}_{d_model}.pkl', 'wb') as file:
                pickle.dump(scaler, file)
                
            torch.save(transformer_model[2].state_dict(),
                       f'trained_models_kmeans/transformer_model_{n_components}_{d_model}.pt')
...
def train_non_k_means_strategy(n_components_list, d_models_list, one_hot_encode_features, targets, num_epochs,
    ...
    for n_components in n_components_list:
        for d_model in d_models_list:
            ...
            # Save the trained models
            os.makedirs('trained_models_nonkmeans', exist_ok=True)
            with open(f'trained_models_nonkmeans/label_reducer_{n_components}_{d_model}.pkl', 'wb') as file:
                pickle.dump(label_reducer, file)

            with open(f'trained_models_nonkmeans/scaler_{n_components}_{d_model}.pkl', 'wb') as file:
                pickle.dump(scaler, file)

            torch.save(transformer_model[2].state_dict(),
                       f'trained_models_nonkmeans/transformer_model_{n_components}_{d_model}.pt')


def main1(sampling_strategy = "k-means"):
...
def main2( models_dir = 'trained_models_nonkmeans'):
    ...
    # Prepare augmented data
    if 'trained_models_kmeans' in models_dir:
        one_hot_encode_features, targets, one_hot_test = prepare_augmented_data(data_file=data_file,
                                                                                id_map_file=id_map_file)
    else:
        one_hot_encode_features, targets, one_hot_test = prepare_augmented_data_mean_only(data_file=data_file,
...
def main3():
    df1 = load_data(r"./trained_models_kmeans_output.csv")
    df2 = load_data(r"./trained_models_nonkmeans_output.csv")
    weights = [0.5 /(0.5+0.25), 0.25 / (0.5+0.25) ]
    df = weights[0] * df1 + weights[1] * df2
...
if use_pre == False:
    # main1("k-means")
    # main2("trained_models_kmeans")
    main1("random")
    main2("trained_models_nonkmeans")
    main3()
else:
    main3()