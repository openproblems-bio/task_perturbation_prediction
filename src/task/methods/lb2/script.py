...
# def load_transformer_model(n_components, input_features, d_model, models_foler='trained_models', device='cpu'):
...
def train_k_means_strategy(n_components_list, d_models_list, one_hot_encode_features, targets, num_epochs,
    ...
    for n_components in n_components_list:
        for d_model in d_models_list:
            ...
            with open(f'trained_models/label_reducer_{n_components}_{d_model}.pkl', 'wb') as file:
                pickle.dump(label_reducer, file)

            with open(f'trained_models/scaler_{n_components}_{d_model}.pkl', 'wb') as file:
                pickle.dump(scaler, file)
                
            torch.save(transformer_model[2].state_dict(),
                       f'trained_models/transformer_model_{n_components}_{d_model}.pt')
...
@torch.no_grad()
def predict_test(data, models, n_components_list, d_list, batch_size, device='cpu'):
...
def main2():
    ...
    # Prepare augmented data
    if 'trained_models' in models_dir:
        one_hot_encode_features, targets, one_hot_test = prepare_augmented_data(data_file=data_file,
                                                                                id_map_file=id_map_file)
    else:
        one_hot_encode_features, targets, one_hot_test = prepare_augmented_data_mean_only(data_file=data_file,
